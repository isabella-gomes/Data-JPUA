...
noiseVLC = 10^(-21);
noiseEveVLC = mean(noiseEveVLC./(gamma.*eta),'all');
...
for i=1:nUsers
    ...
    elseif alocUsers(i)==0
        V = Votimo;
        V(:,k)=[];
        CLUser(i) = log2(1+(((hRF(:,k)'*Votimo(:,k)).^2)/3)./(sum((((hRF(:,k)'*V)).^2)/3)+ noiseRF));
        CEUser(i) = log2(1+(((heRF'*Votimo(:,k)).^2)/3)./(sum(((heRF'*V).^2)/3)+noiseRF));
    ...
    if alocUsers(i)==0 && UsersVLC<nLeds
        ...
        if UsersRF~=0
            ...
            for z=1:UsersRF
                Vaux = Votimoaux;
                Vaux(:,z)=[];
                CLUseriRF(z) = log2(1+(((hRFi(:,z)'*Votimoaux(:,z)).^2)/3)./(sum((((hRFi(:,z)'*Vaux)).^2)/3)+ noiseRF));
                CEUseriRF(z) = log2(1+(((heRF'*Votimoaux(:,z)).^2)/3)./(sum(((heRF'*Vaux).^2)/3)+noiseRF));
    ...
    elseif alocUsers(i)==1 && UsersRF<nAntenas
        ...
        CSRFaux=zeros(1,UsersRF);
            for z=1:UsersRF
                Vaux = Votimoi;
                Vaux(:,z)=[];
                CLUseriRF(z) = log2(1+(((hRFi(:,z)'*Votimoi(:,z)).^2)/3)./(sum((((hRFi(:,z)'*Vaux)).^2)/3)+ noiseRF));
                CEUseriRF(z) = log2(1+(((heRF'*Votimoi(:,z)).^2)/3)./(sum(((heRF'*Vaux).^2)/3)+noiseRF));
                CSRFaux(z)=max(CLUseriRF(z)-CEUseriRF(z),0);
            end
        ...
        if UsersVLC~=0
            ...
            for f=1:UsersVLC
                Waux = Wotimoaux;
                Waux(:,f)=[];
                CLUseriVLC(f) = log2((sum(alphaki.*((hVLCi(:,f))'*Wotimoaux).^2)+1)/(sum(betaki.*(((hVLCi(:,f))'*Waux).^2))+ 1))/2;
                CEUseriVLC(f) = log2((sum(alphae.*((gEveVLC)'*Wotimoaux).^2)+1)/(sum(betae.*(((gEveVLC)'*Waux).^2))+ 1))/2;
